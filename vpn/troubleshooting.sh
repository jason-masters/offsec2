...
if (dmidecode | grep -iq vmware); then
  ...
  echo -e "    See: https://support.offensive-security.com/#!pwk-kali-vm.md"
  sleep 2s
else
...
files=$(find . -name '*.ovpn' 2>/dev/null | wc -l)
if [[ "${files}" == "1" ]]; then
  ...
  ls -lah *.ovpn | tee -a troubleshoot.log
  echo -e "" | tee -a troubleshoot.log
  ls -lah | tee -a troubleshoot.log
  pwd | tee -a troubleshoot.log
  sleep 2s
...
echo -e "${BLUE}[+]${RESET} Please ${BOLD}send the log file './troubleshoot.log'${RESET} along with the ${BOLD}output from the OpenVPN window${RESET}, with ${BOLD}your OSID${RESET} to '${BOLD}help@offensive-security.com${RESET}'."
sleep 3s
